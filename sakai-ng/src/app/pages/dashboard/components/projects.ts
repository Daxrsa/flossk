    ...
    template: `
        ...
        <div class="projects mb-6">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-2xl font-semibold text-surface-900 dark:text-surface-0 m-0">API Projects</h3>
                <p-tag [value]="apiProjects.length.toString()" severity="info"></p-tag>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div *ngFor="let project of apiProjects" class="bg-surface-0 dark:bg-surface-900 border border-surface rounded-xl p-5 hover:shadow-lg transition-all duration-200">
                    <div class="flex items-start justify-between mb-3">
                        <h4 class="text-lg font-semibold text-surface-900 dark:text-surface-0 m-0">{{ project.title }}</h4>
                        <p-tag [value]="project.status" [severity]="project.status === 'Completed' ? 'success' : project.status === 'InProgress' ? 'info' : 'warn'"></p-tag>
                    </div>
                    <p class="text-surface-600 dark:text-surface-400 text-sm mb-4">{{ project.description }}</p>
                    ...
                    <div class="mb-3">
                        ...
                        <p-progressbar [value]="project.progressPercentage" [showValue]="false" [style]="{'height': '6px'}"></p-progressbar>
                    </div>
                    
                    <div class="flex flex-wrap gap-4 text-xs text-surface-500 dark:text-surface-400 mb-3">
                        <div class="flex items-center gap-1">
                            <i class="pi pi-calendar"></i>
                            <span>{{ project.startDate | date:'MMM d, yyyy' }} - {{ project.endDate | date:'MMM d, yyyy' }}</span>
                        </div>
                    </div>
                    
                    <div class="flex flex-wrap gap-3 mb-3">
                        <div class="flex items-center gap-1 text-xs bg-surface-100 dark:bg-surface-800 px-2 py-1 rounded-full">
                    ...
                    <div class="flex items-center justify-between text-xs text-surface-500 dark:text-surface-400">
                        <div class="flex items-center gap-2">
                            <p-avatar [label]="project.createdByFirstName?.charAt(0) + project.createdByLastName?.charAt(0)" shape="circle"></p-avatar>
            ...
            <div *ngIf="apiProjects.length === 0" class="flex items-center justify-center p-8 bg-surface-50 dark:bg-surface-800 rounded-xl">
                <i class="pi pi-spin pi-spinner text-2xl text-primary mr-3"></i>
                <span class="text-surface-600 dark:text-surface-400">Loading projects...</span>
            </div>
        </div>
